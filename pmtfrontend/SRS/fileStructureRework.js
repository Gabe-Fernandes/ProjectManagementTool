$(function() {
  ...
  function sortContainer(dirContainer) {
    ...
    for (let i = 0; i < sortedDirs.length; i++) {
      ...
      const imgToPush = sortedDirs[i].fsItem.find("img:first");
      const scale = imgToPush.parents(".dir-container").length;
      const indentationSize = 2;
      imgToPush.css("margin-left", `${scale * indentationSize}vw`);
    }
    for (let i = 0; i < sortedFiles.length; i++) {
      dirContainer.append(sortedFiles[i].fsItem);
      const imgToPush = sortedFiles[i].fsItem.find("img:first");
      const scale = imgToPush.parents(".dir-container").length;
      const indentationSize = 2;
      imgToPush.css("margin-left", `${scale * indentationSize}vw`);
    }
