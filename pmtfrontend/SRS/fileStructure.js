$(function() {
  ...
  function setFileStructureEvents(fileStructureItem) {
    ...
    fileStructureItem.on("dragend", ()=>{
      ...
      const parentDir = fileStructureItem.prevAll(".dir:first");
      const newTier = parseInt(parentDir.attr("data-tier")) + 1;
      fileStructureItem.attr("data-tier", newTier);
      if (fileStructureItem.hasClass("dir")) {
        const dirContents = getDirContents(fileStructureItem);
        for (let i = 0; i < dirContents.length; i++) {
          dirContents[i].attr("data-tier", );
        }
      }
      setFileStructureIndentation(fileStructureItem);
    });
  }
  ...
  function newItem(type, src, name) {
    ...
    if (type === "dir") {
      ...
      `).on("click", (event)=> toggleDirArrowDirection(event));
    }
  ...
  $("#ctxMenuDelete").on("click", ()=>{
    ...
    const dirContents = getDirContents(target);
    for (let i = 0; i < dirContents.length; i++) {
      dirContents[i].remove();
    }
    if (!target.is("#solutionDir")) {
      target.remove();
    }
  });
  ...
  // open/close Dir (arrow animation)
  function toggleDirArrowDirection(event) {
    const dirBtn = $(event.target);
    const dir = $(event.target).parent(".dir");
    ...
    if (dirBtn.hasClass("closed-dir-btn")) {
      ...
      for (let i = 0; i < dirContents.length; i++) {
        dirContents[i].removeClass("hide");
      }
    }
    else {
      // collapse
  ...
  function getDirContents(dir) {
    ...
    while (dirDataTier < nextItemDataTier) {
      if (!previousItem.is(dir)) {
        dirContents.push(previousItem);
      }
